...
class CBIR:
    def __init__(
        self, feature_extractor: FeatureExtractor, feature_store: FeatureStore
    ):
    ...
    @abstractmethod
    def indexing(
        self, images: List[np.ndarray | os.PathLike] | np.ndarray | os.PathLike
    ) -> None:
        if isinstance(images, np.ndarray) and len(images.shape) == 3:
            ...
            assert isinstance(
                images[0], (np.ndarray, str, os.PathLike)
